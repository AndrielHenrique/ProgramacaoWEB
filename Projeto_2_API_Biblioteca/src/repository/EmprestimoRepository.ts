...
export class EmprestimoRepository {
    ...
    private async createTable() {
        const query = `
        CREATE TABLE IF NOT EXISTS estoque.Emprestimo (
    ...
    async inserirEmprestimo(emprestimo: EmprestimoEntity): Promise<EmprestimoEntity> {
        const query = "INSERT INTO estoque.Emprestimo (idLivro, usuarioID, dataEmprestimo, dataDevolucao) VALUES (?, ?, ?, ?)";
    ...
    async atualizarEmprestimo(emprestimo: EmprestimoEntity): Promise<EmprestimoEntity> {
        const query = "UPDATE estoque.Emprestimo SET idLivro = ?, usuarioID = ?, dataEmprestimo = ?, dataDevolucao = ? WHERE id = ?";
    ...
    async deletarEmprestimo(emprestimoId: number): Promise<void> {
        const query = "DELETE FROM estoque.Emprestimo WHERE id = ?";
    ...
    async filterEmprestimoPorId(id: number): Promise<EmprestimoEntity> {
        const query = "SELECT * FROM estoque.Emprestimo WHERE id = ?";
        ...
        try {
            ...
            console.error(`Erro ao buscar o emprestimo de ID ${id}: ${err}`);
            throw err;
        }
    }

    async filterEmprestimosPorUsuario(usuarioID: number): Promise<EmprestimoEntity[]> {
        const query = "SELECT * FROM estoque.Emprestimo WHERE usuarioID = ?";
    ...
    async filterEmprestimosPorLivro(idLivro: number): Promise<EmprestimoEntity[]> {
        const query = "SELECT * FROM estoque.Emprestimo WHERE idLivro = ?";
    ...
    async listarTodosEmprestimos(): Promise<EmprestimoEntity[]> {
        const query = "SELECT * FROM estoque.Emprestimo";
