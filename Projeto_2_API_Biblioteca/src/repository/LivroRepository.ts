...
export class LivroRepository {
    ...
    private async createTable() {
        const query = `
        CREATE TABLE IF NOT EXISTS estoque.Livro (
    ...
    async deleteLivro(livroId: number): Promise<void> {
        try {
            const query = "DELETE FROM estoque.Livro WHERE id = ?";
    ...
    async filterLivro(id: number): Promise<LivroEntity> {
        const query = "SELECT * FROM estoque.Livro where id = ?";
    ...
    async filterLivroByName(name: string): Promise<LivroEntity[]> {
        const query = "SELECT * FROM estoque.Livro where name = ?";
    ...
    async filterAllLivros(): Promise<LivroEntity[]> {
        const query = "SELECT * FROM estoque.Livro";
    ...
    async getLivroByAll(name?: string, autor?: string, categoriaID?: number, id?: number): Promise<LivroEntity[]> {
        let query = "SELECT * FROM estoque.Livro WHERE";
        const params: any[] = [];
